...
MINIMAX_MCP_SERVER_NAME = "minimax_search"
MINIMAX_MCP_SEARCH_TOOL_NAME = "web_search"
...
async def tavily_search(
    ...
    summarization_model = init_chat_model(
        model=get_model_name_for_init(configurable.summarization_model),
        max_tokens=configurable.summarization_model_max_tokens,
        api_key=model_api_key,
        base_url=get_base_url_for_model(configurable.summarization_model),
        extra_body=get_extra_body_for_model(configurable.summarization_model),
        tags=["langsmith:nostream"]
    )
    summarization_model = with_structured_output_for_model(
        summarization_model,
        Summary,
...
async def load_mcp_tools(
    ...
    include_minimax_search: bool = False,
) -> list[BaseTool]:
    ...
    Args:
        ...
        include_minimax_search: Whether to load MiniMax's web_search MCP tool
    ...
    # Step 2: Build configured MCP server connection if present
    config_valid = bool(
        configurable.mcp_config and 
        configurable.mcp_config.tools and 
        (
            (
                (configurable.mcp_config.transport or "streamable_http") == "stdio" and
                configurable.mcp_config.command
            ) or
            (
                (configurable.mcp_config.transport or "streamable_http") != "stdio" and
    ...
    if config_valid:
        transport = configurable.mcp_config.transport or "streamable_http"
        if transport == "stdio":
            mcp_server_config["configured"] = {
                "transport": "stdio",
                "command": configurable.mcp_config.command,
                "args": configurable.mcp_config.args or [],
                "env": configurable.mcp_config.env,
            }
        else:
        ...
        tool_names_by_server["configured"] = set(configurable.mcp_config.tools)

    # Step 3: Add MiniMax's local MCP web search server when selected as search API
    if include_minimax_search:
        mcp_server_config[MINIMAX_MCP_SERVER_NAME] = get_minimax_mcp_server_config(config)
        tool_names_by_server[MINIMAX_MCP_SERVER_NAME] = {MINIMAX_MCP_SEARCH_TOOL_NAME}

    if not mcp_server_config:
        return []

    # Step 4: Load, filter, and configure tools from each MCP server
    client = MultiServerMCPClient(mcp_server_config)
    configured_tools = []
    configured_tool_names = set(existing_tool_names)
    for server_name in mcp_server_config:
        try:
            available_mcp_tools = await client.get_tools(server_name=server_name)
        except Exception as e:
            warnings.warn(
                f"Failed to load MCP tools from '{server_name}': {e.__class__.__name__}"
            )
            continue

        allowed_tool_names = tool_names_by_server[server_name]
        for mcp_tool in available_mcp_tools:
            # Skip tools with conflicting names
            if mcp_tool.name in configured_tool_names:
                warnings.warn(
            ...
            if server_name == MINIMAX_MCP_SERVER_NAME:
                enhanced_tool.metadata = {
                    **(enhanced_tool.metadata or {}),
                    "type": "search",
                    "name": MINIMAX_MCP_SEARCH_TOOL_NAME,
                }
            configured_tools.append(enhanced_tool)
            configured_tool_names.add(mcp_tool.name)
...
async def get_search_tool(search_api: SearchAPI):
    ...
    elif search_api == SearchAPI.MINIMAX_MCP:
        # MiniMax search is loaded through the MCP tool loader below.
        return []
...
async def get_all_tools(config: RunnableConfig):
    ...
    # Add MCP tools if configured, including MiniMax search when selected
    mcp_tools = await load_mcp_tools(
        config,
        existing_tool_names,
        include_minimax_search=search_api == SearchAPI.MINIMAX_MCP,
    )
...
def get_non_empty_env(key: str, default: Optional[str] = None) -> Optional[str]:
    """Return an environment variable only when it has a non-empty value."""
    value = os.getenv(key, default)
    return value if value else None

def get_model_name_for_init(model_name: str) -> str:
    """Return the LangChain init_chat_model model name for configured providers."""
    provider, _, model_id = model_name.partition(":")
    if provider.lower() in {"qwen", "minimax"} and model_id:
...
def get_api_key_for_model(model_name: str, config: RunnableConfig):
    ...
    if should_get_from_config.lower() == "true":
        ...
        if model_name.startswith("openai:"):
            ...
            return api_keys.get("QWEN_API_KEY") or api_keys.get("DASHSCOPE_API_KEY")
        elif model_name.startswith("minimax:"):
            return api_keys.get("MINIMAX_API_KEY")
        ...
        if model_name.startswith("openai:"): 
            return get_non_empty_env("OPENAI_API_KEY")
        elif model_name.startswith("deepseek:"):
            return get_non_empty_env("DEEPSEEK_API_KEY")
        elif model_name.startswith("qwen:"):
            return get_non_empty_env("QWEN_API_KEY") or get_non_empty_env("DASHSCOPE_API_KEY")
        elif model_name.startswith("minimax:"):
            return get_non_empty_env("MINIMAX_API_KEY")
        elif model_name.startswith("anthropic:"):
            return get_non_empty_env("ANTHROPIC_API_KEY")
        elif model_name.startswith("google"):
            return get_non_empty_env("GOOGLE_API_KEY")
        return None

def get_base_url_for_model(model_name: str):
    """Get provider-specific base URL for OpenAI-compatible models."""
    model_name = model_name.lower()
    if model_name.startswith("deepseek:"):
        return get_non_empty_env("DEEPSEEK_BASE_URL", "https://api.deepseek.com")
    if model_name.startswith("qwen:"):
        return (
            get_non_empty_env("QWEN_BASE_URL")
            or get_non_empty_env("DASHSCOPE_BASE_URL")
            or "https://dashscope.aliyuncs.com/compatible-mode/v1"
        )
    if model_name.startswith("minimax:"):
        return get_non_empty_env("MINIMAX_BASE_URL", "https://api.minimax.io/v1")
    if model_name.startswith("openai:"):
        return get_non_empty_env("OPENAI_BASE_URL")
    return None

def get_extra_body_for_model(model_name: str):
    """Get provider-specific extra request body parameters."""
    if model_name.lower().startswith("deepseek:"):
        return {"thinking": {"type": "disabled"}}
    return None

def get_minimax_api_key(config: RunnableConfig) -> Optional[str]:
    """Get the MiniMax API key from environment or runtime config."""
    should_get_from_config = os.getenv("GET_API_KEYS_FROM_CONFIG", "false")
    if should_get_from_config.lower() == "true":
...
def get_minimax_api_host() -> str:
    """Get the MiniMax API host expected by MiniMax MCP servers."""
    explicit_host = get_non_empty_env("MINIMAX_API_HOST")
    if explicit_host:
        return explicit_host.rstrip("/")

    base_url = get_non_empty_env("MINIMAX_BASE_URL")
    if base_url:
        return re.sub(r"/v\d+/?$", "", base_url.rstrip("/"))

    return "https://api.minimax.io"

def get_minimax_mcp_server_config(config: RunnableConfig) -> dict[str, Any]:
    """Build the stdio MCP server config for MiniMax web search."""
    env = {
        "MINIMAX_API_HOST": get_minimax_api_host(),
    }

    api_key = get_minimax_api_key(config)
    if api_key:
    ...
    for optional_key in (
        ...
        optional_value = get_non_empty_env(optional_key)
        if optional_value:
            env[optional_key] = optional_value

    args_env = get_non_empty_env("MINIMAX_MCP_ARGS")
    return {
        "transport": "stdio",
        "command": get_non_empty_env("MINIMAX_MCP_COMMAND", "uvx"),
        "args": shlex.split(args_env) if args_env else ["minimax-coding-plan-mcp", "-y"],
        "env": env,
    }
...
def _extract_json_object(text: str) -> Any:
    ...
    candidates = [block.strip() for block in fenced_blocks] + [text]
    ...
    raise ValueError(f"Could not extract JSON object from model output: {text[:500]}")

def _parse_structured_response(response: Any, schema: Any) -> Any:
    """Parse a provider response into the requested structured schema."""
...
def with_structured_output_for_model(model: Any, schema: Any, model_name: str):
    """Configure structured output using a provider-compatible method."""
    provider = get_model_provider(model_name)
    env_method = get_non_empty_env(f"{provider.upper()}_STRUCTURED_OUTPUT_METHOD")
    if env_method:
        return model.with_structured_output(schema, method=env_method)
    if provider == "deepseek":
        return model.with_structured_output(schema, method="json_mode")
    if provider == "minimax":
        return model | RunnableLambda(lambda response: _parse_structured_response(response, schema))
...
def get_tavily_api_key(config: RunnableConfig):
    ...
    if should_get_from_config.lower() == "true":
        ...
        return get_non_empty_env("TAVILY_API_KEY")
